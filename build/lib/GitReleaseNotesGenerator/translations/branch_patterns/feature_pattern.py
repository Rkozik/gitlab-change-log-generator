...
class FeaturePattern(Translation):
    ...
    def translate(self):
        ...
        name = self.__regex(self.phrase).group(2) if 2 == len(self.__regex(self.phrase).groups()) else ""
        return {"id": id, "name": name}

    def __regex(self, phrase):
        regex = re.search(r'((?<=feature\/)[0-9]+)(.*)', phrase)
